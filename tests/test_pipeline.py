...
# ---------------------------------------------------------------- payouts and debt cross-check
def test_ttm0_treats_untagged_quarters_as_zero():
    assert p.ttm0({"2024Q4": 5.0, "2024Q2": 3.0}, "2024Q4") == 8.0
    assert p.ttm0(None, "2024Q4") == 0.0


def test_payouts_push_coverage_past_the_line_without_changing_status():
    f = _fund([80.0] * 12)  # capex 320 vs OCF 400 TTM -> 80%
    labs = _quarters(12)
    for t in CFG["core"]:
        f[t]["buybacks"] = {l: 20.0 for l in labs}
        f[t]["dividends"] = {l: 5.0 for l in labs}   # payouts 100 TTM -> 105% all-in
    f["AMZN"].pop("buybacks"); f["AMZN"].pop("dividends")  # a non-payer must not break the aggregate
    r = p.compute(f, {}, {}, [], CFG)
    s = {x["key"]: x for x in r["signals"]}
    assert s["capex_ocf"]["state"] == "clear"
    assert abs(r["history"][-1]["capex_payout_ocf"] - (1280 + 300) / 1600) < 1e-9
    amzn = [row for row in s["capex_ocf"]["table"]["rows"] if row[0] == "AMZN"][0]
    assert amzn[2] == "$0.0B" and amzn[6] == "80%"


def test_debt_cross_check_consistent_when_flows_match():
    debt = [1000.0e9 + 10e9 * i for i in range(12)]
    f = _fund([50e9] * 12, ocf=100e9, debt_path=debt)
    for t in CFG["core"]:
        f[t]["debt_proceeds"] = {l: 10e9 for l in _quarters(12)}
    r = p.compute(f, {}, {}, [], CFG)
    s = {x["key"]: x for x in r["signals"]}
    rows = [x for x in s["debt_share"]["table"]["rows"] if x[1] == "Core"]
    assert all(x[7] == "Consistent" for x in rows)
    assert "All core companies reconcile" in s["debt_share"]["detail"]


def test_debt_cross_check_flags_disagreement_and_tag_switch():
    debt = [1000.0e9] * 8 + [1100.0e9] * 4          # +$100B on the balance sheet
    f = _fund([50e9] * 12, ocf=100e9, debt_path=debt)  # no issuance on the cash-flow statement
    labs = _quarters(12)
    f["MSFT"]["debt_lt"] = {l: 1.0 for l in labs[-4:]}  # combined tag only in recent quarters
    f["MSFT"]["debt_lt_noncurrent"] = {l: 1.0 for l in labs[:-4]}
    r = p.compute(f, {}, {}, [], CFG)
    rows = {x[0]: x for x in [s for s in r["signals"] if s["key"] == "debt_share"][0]["table"]["rows"]}
    assert rows["MSFT"][7].startswith("Tag basis changed")
    assert rows["GOOGL"][7].startswith("Balance sheet and cash flow disagree")
