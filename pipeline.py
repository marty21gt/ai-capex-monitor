...
def fetch_fundamentals(sec: SEC, ticker: str, meta: dict, cfg: dict):
    ...
    log("INFO", f"{ticker}: buybacks via {used.get('buybacks')}, dividends via {used.get('dividends')}, "
                f"debt proceeds via {used.get('debt_proceeds')}, repayments via {used.get('debt_repayments')}, "
                f"CP/short-term net via {used.get('cp_net')}")
    for i in issues:
...
def ttm0(series: dict[str, float], lab: str) -> float:
    """TTM for sparse cash-flow items (buybacks, dividends, debt flows).

    Companies often leave zero-activity quarters untagged, so a missing quarter
    counts as zero here. Only use for flows where absence genuinely means none;
    the per-company tables on the dashboard show every input for checking.
    """
    return sum((series or {}).get(qoffset(lab, -i), 0.0) for i in range(4))


def bil(v) -> str:
    if v is None:
        return "n/a"
    return f"-${abs(v) / 1e9:,.1f}B" if v < 0 else f"${v / 1e9:,.1f}B"


def debt_basis(f: dict, lab: str) -> str | None:
    """Which balance-sheet tags produced total_debt for this quarter."""
    if lab in f.get("debt_lt", {}):
        base = "LongTermDebt"
    elif lab in f.get("debt_lt_noncurrent", {}) or lab in f.get("debt_lt_current", {}):
        base = "Noncurrent + current"
    else:
        return None
    extras = [n for k, n in (("commercial_paper", "CP"), ("short_term_borrowings", "ST borrowings")) if lab in f.get(k, {})]
    return base + (" + " + " + ".join(extras) if extras else "")


def debt_breakdown(fund: dict, tickers: list[str], lab_for: dict[str, str | None], tier: str) -> list[list[str]]:
    """Per-company debt rows with an independent cash-flow cross-check."""
    rows = []
    for t in tickers:
        f = fund.get(t, {})
        lab = lab_for.get(t)
        td = f.get("total_debt", {})
        if not lab or lab not in td or qoffset(lab, -4) not in td:
            rows.append([t, tier, lab or "n/a", "n/a", "n/a", "n/a", "n/a", "Not enough quarters"])
            continue
        prev = qoffset(lab, -4)
        bs = td[lab] - td[prev]
        cf = (ttm0(f.get("debt_proceeds"), lab) - ttm0(f.get("debt_repayments"), lab)
              + ttm0(f.get("cp_net"), lab))
        cap = ttm(f.get("capex", {}), lab)
        b_now, b_prev = debt_basis(f, lab), debt_basis(f, prev)
        if b_now != b_prev:
            check = f"Tag basis changed ({b_prev} to {b_now})"
        elif abs(bs - cf) > max(0.25 * max(abs(bs), abs(cf)), 2e9):
            check = "Balance sheet and cash flow disagree: check the 10-Q"
        else:
            check = "Consistent"
        rows.append([t, tier, lab, bil(bs), bil(cf), "n/a" if not cap else pct(bs / cap), b_now or "n/a", check])
    return rows
...
def compute(fund: dict, seg: dict, mkt: dict, alerts: list, cfg: dict) -> dict:
    ...
    for lab in sorted(set(C) & set(O)):
        ...
        pay_t = sum(ttm0(fund.get(t, {}).get("buybacks"), lab) + ttm0(fund.get(t, {}).get("dividends"), lab)
                    for t in core)
        rows.append({"q": lab, "capex_ttm": cap_t, "ocf_ttm": ocf_t, "payouts_ttm": pay_t,
                     "capex_ocf": ratio(cap_t, ocf_t), "capex_dna": ratio(cap_t, dna_t),
                     "capex_payout_ocf": ratio(cap_t + pay_t, ocf_t),
    ...
    vp = latest.get("capex_payout_ocf")
    table1 = None
    if lq:
        trows = []
        for t in core:
            f = fund.get(t, {})
            cap, ocf = ttm(f.get("capex", {}), lq), ttm(f.get("ocf", {}), lq)
            bb, dv = ttm0(f.get("buybacks"), lq), ttm0(f.get("dividends"), lq)
            trows.append([t, bil(cap), bil(bb), bil(dv), bil(ocf),
                          "n/a" if not ocf or cap is None else pct(cap / ocf),
                          "n/a" if not ocf or cap is None else pct((cap + bb + dv) / ocf)])
        table1 = {"caption": f"Trailing 12 months to {lq}",
                  "columns": ["Company", "Capex", "Buybacks", "Dividends", "Operating cash flow",
                              "Capex ÷ OCF", "Capex + payouts ÷ OCF"],
                  "rows": trows}
    signals.append({
        ...
        "detail": ("Core hyperscalers, trailing 12 months" + (f", {direction} vs. two quarters ago" if direction else "")
                   + (f". Including buybacks and dividends: {vp * 100:.0f}% of operating cash flow"
                      + (" (past the self-funded line)" if vp >= 1 else "") if vp is not None else "")
                   + ". The status rule uses capex alone"
    ...
    lab_for = {t: lq for t in core}
    for t in canary:
        td = fund.get(t, {}).get("total_debt", {})
        lab_for[t] = max(td) if td else None
    brows = debt_breakdown(fund, core, lab_for, "Core") + debt_breakdown(fund, canary, lab_for, "Canary")
    core_rows = [r for r in brows if r[1] == "Core"]
    flagged = [r[0] for r in core_rows if r[7] != "Consistent"]
    bs_sum = sum(fund[t]["total_debt"][lq] - fund[t]["total_debt"][qoffset(lq, -4)] for t in core
                 if lq and lq in fund.get(t, {}).get("total_debt", {})
                 and qoffset(lq, -4) in fund.get(t, {}).get("total_debt", {})) if lq else None
    cf_sum = sum(ttm0(fund.get(t, {}).get("debt_proceeds"), lq) - ttm0(fund.get(t, {}).get("debt_repayments"), lq)
                 + ttm0(fund.get(t, {}).get("cp_net"), lq) for t in core) if lq else None
    cross = (f" Cross-check: balance sheets show {bil(bs_sum)} of new core debt; cash-flow statements show "
             f"{bil(cf_sum)} of net issuance."
             + (f" Verify {', '.join(flagged)} before relying on this reading." if flagged else " All core companies reconcile.")
             ) if lq else ""
    signals.append({
        ...
        "detail": ("Change in core gross debt over 12 months as a share of 12-month capex"
                   ...
                   + ". Excludes leases and off-balance-sheet vehicles; see filings below." + cross),
        "table": {"caption": "12-month change by company. Net issuance comes from the cash-flow statement, as an independent check on the balance sheet.",
                  "columns": ["Company", "Tier", "Quarter", "Debt change", "Net issuance", "Share of own capex", "Balance-sheet tags", "Check"],
                  "rows": brows},
