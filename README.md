...
1. **Capex ÷ operating cash flow** (core: MSFT, GOOGL, AMZN, META). Above 100% means the buildout needs outside money. The detail and chart also show capex plus buybacks and dividends ÷ OCF; the status rule uses capex alone.
2. **Canary market stress.** ORCL and CRWV 63-day return relative to QQQ. Fastest signal, noisiest.
3. **New debt ÷ capex.** 12-month change in core gross debt as a share of 12-month capex. Confirms #1. A per-company table cross-checks the balance-sheet change against net issuance from the cash-flow statement and flags tag-basis switches.
...
- Buybacks, dividends, and debt flows treat an untagged quarter as zero, because companies often omit zero-activity lines. The per-company tables show every input so gaps are visible.
